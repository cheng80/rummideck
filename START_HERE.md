...
현재 UI 작업 메모:
- 우측 덱은 별도 오브젝트를 두지 않고, 하단 정보 줄의 `Deck x/52` 표기와 화면 밖 시작점 드로우 연출 기준으로 정리했다.
- 드로우 애니메이션 중에는 최상단 투명 입력 차단 레이어가 동작한다. 제출/점수/클리어 연출도 같은 락 구조를 쓰고 있다.
- `Play Hand` 후 흐름은 `제출 -> 점수 연출 -> 최종 점수 반영 -> (리필 또는 상점/게임오버)` 순서로 재구성했다.
- 점수 연출은 별도 복제 타일이 아니라 **제출된 실제 중앙 타일 스냅샷** 위에 하이라이트 박스와 숫자 팝업을 얹는 구조로 바뀌었다.
- 현재 가장 많이 남은 UI 과제는 **점수 팝업 위치/타이밍/Jester 반응** 튜닝이다.
- 현재 우선순위는 **기본 전투 연출 완성**이다. `GameView` 모듈 분리와 `Riverpod` 전환은 그 이후 리팩토링 단계에서 진행한다.

다음 작업 우선순위:
1. 점수 연출 마감: 카드별 숫자 팝업 위치, 속도, 최종 점수 표시 타이밍, Jester 반응 추가
2. 리필/클리어 전환 마감: 점수 연출 후 드로우/상점 전환 리듬 최종 조정
3. 완전 순회 검증: 첫 전투 ~ 1차 보스까지 실제 플레이 감각 점검
4. 그 다음에만 `GameView` 분해와 `Riverpod` 리팩토링
