...
## 1. 목적
이 문서는 Balatro의 **Joker 시스템을 1:1로 이식**하여, 프로젝트에서 사용하는 **변칙 타일(Jester)**의 규칙·용어·가격·희귀도·에디션·스티커·트리거를 완전하게 정의한다.

> 원칙: **이름만 바꾸고(조커 → 변칙 타일), 구조와 수치는 유지한다.**
...
각 변칙 타일은 추가 속성(Edition)을 가질 수 있다.
...
모든 변칙 타일 효과는 아래 4가지 중 하나로 귀속된다.
...
### 원칙 3
조커 효과는 점수 계산 파이프라인의 각 단계에 정확히 삽입

### 원칙 4
조커는 핸드보다 후반 영향력이 커야 한다
...
👉 **변칙 타일은 점수를 더하는 시스템이 아니라, 점수 계산 규칙을 바꾸는 엔진이다.**
